...
int wait(int* status)
{
    return (int)syscall4(SYS_wait4, -1, (long)status, 0, 0);
}
...
unsigned int sleep(unsigned int seconds)
{
    ...
    while (syscall2(SYS_nanosleep, (long)&req, (long)&rem) < 0) {
        slept += (unsigned int)(req.tv_sec - rem.tv_sec);
        req = rem;
...
{
    return (int)syscall2(SYS_chmod, (long)pathname, mode);
}

int chown(const char* pathname, uid_t owner, gid_t group)
{
    return (int)syscall3(SYS_chown, (long)pathname, owner, group);
}
...
static char* environ_table[64] = { NULL };
...
char* getenv(const char* name)
{
    ...
    for (int i = 0; i < environ_count; i++) {
        if (environ_table[i]) {
            if (strncmp(environ_table[i], name, len) == 0
                && environ_table[i][len] == '=') {
                return environ_table[i] + len + 1;
            }
        }
    }
    return NULL;
...
int setenv(const char* name, const char* value, int overwrite)
{
    ...
    size_t len = strlen(name);
    for (int i = 0; i < environ_count; i++) {
        if (environ_table[i] && strncmp(environ_table[i], name, len) == 0
            && environ_table[i][len] == '=') {
            if (!overwrite) return 0;
            
            size_t total = len + 1 + strlen(value) + 1;
            if (total > 256) return -1;
            strcpy(environ_table[i] + len + 1, value);
            return 0;
        }
    }
    if (environ_count >= 64) return -1;
    size_t total = len + 1 + strlen(value) + 1;
    if (total > 256) return -1;
    static char env_storage[64][256];
    char* slot = env_storage[environ_count];
    strcpy(slot, name);
    slot[len] = '=';
    strcpy(slot + len + 1, value);
    environ_table[environ_count++] = slot;
    return 0;
}
